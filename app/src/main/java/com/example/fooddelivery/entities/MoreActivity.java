...
public class MoreActivity extends AppCompatActivity implements OnItemClickListener {
    ...
    private SearchAndMoreViewModel MoreViewModel;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        backFromMoreImage.setOnClickListener(v -> {
            Intent intent = new Intent(this, MainActivity.class);
            startActivity(intent);
            finish();
        });

        MoreViewModel = new ViewModelProvider(this).get(SearchAndMoreViewModel.class);
        ...
        MoreViewModel.getListByCategory(category.trim()).observe(this, postList -> {
            moreAdapter.submitList(postList);
            Log.d("LIST", String.valueOf(postList.size()));
            moreAdapter.notifyDataSetChanged();
        });
        moreProgressBar.setVisibility(View.VISIBLE);
        MoreViewModel.getIsCategoryPostsFetched().observe(this, isFetched ->{
            if (isFetched){
